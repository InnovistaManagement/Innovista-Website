(function($){
  ...
  function initMap() {
    if($('#map').length){
      ...
      var marker = new google.maps.Marker({
        position: myLatlng,
        map: map,
        icon: '/themes/custom/comets/images/marker.svg',
        title: 'We are here!'
      });
  ...
  function initGeneral () {
    ...
    $('.path-frontpage #topnav .navigation-menu > li > a').attr('data-scroll', 'true');
  ...
  function fixFormButton(){
    $('.yamlform-submission-form .form-submit').addClass('btn btn-color');

  }

  function commonFixes(){
    //$('.clients img').addClass('client-image');

    $('.team-member > div').on('click',function(){
      var t = $(this);
      t.parent().find('[data-lity]').click();    
    });
    // relocate language switcher

    $('.language-switcher-language-url > ul.links > li:not(.is-active)').appendTo('#navigation .navigation-menu');
    $('#navigation .language-link').each(function(){
      var t = $(this);
      t.text(t.attr('hreflang'));
    });

    $('.path-frontpage #navigation .navigation-menu > li:not(.en):not(.de) > a').each(function(){
      var t = $(this);
      t.attr('href',t.attr('href').replace('/de','').replace('/en',''));
    });
